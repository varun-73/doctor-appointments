...
const DoctorAppointments = () => {
  ...
  const getAppointmentsData = async () => {
    try {
      ...
      const response = await axios.get('https://doctor-appointment-lqtk.onrender.com/api/doctor/get-appointments-by-doctor-id', {
        headers: {
  ...
  const changeAppointmentStatus = async (record, status) => {
    try {
      dispatch(showLoading());
      const response = await axios.post('https://doctor-appointment-lqtk.onrender.com/api/doctor/change-appointment-status', {
  ...
  return (
    <Layout>
      <h1 className='page-header'>Appointments</h1><hr />
